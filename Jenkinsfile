    ...
    stages {
        ...
        stage('build docker image from python code'){
             steps {
                ...
                sh 'sudo docker tag newimage karthick88/newimage:latest'
                sh 'sudo docker tag newimage karthick88/newimage:${BUILD_NUMBER}'
            }
        }
        
        stage ('Push the Docker image') {
            steps {
                sh 'sudo docker image push karthick88/newimage:latest'
                sh 'sudo docker image push karthick88/newimage:${BUILD_NUMBER}'
            }
        }
